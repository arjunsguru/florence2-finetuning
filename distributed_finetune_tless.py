...
import wandb
from tless_dataset import TLESSDataset
from pbr_dataset import PBRDataset
...
def train_model(rank, world_size, batch_size=6, use_lora=False, epochs=10, lr=1e-6, eval_steps=10, run_name=None, max_val_item_count=1000):
    ...
    if rank == 0:  # Only initialize wandb in the main process
        wandb.init(project="TLess-finetune", name=run_name)
        wandb.config.update({
            "dataset": "BOP",
            "batch_size": batch_size,
            "use_lora": use_lora,
    ...
    train_dataset = PBRDataset("/home/scratch.driveix_50t_3/input/aguru/train_pbr")
    print("Train dataset len is")
    print(len(train_dataset))
    val_dataset = TLESSDataset("/home/scratch.driveix_50t_3/input/aguru/t-less_v2", "test")
    val_datasets = {"tless": val_dataset}
    print("Val dataset len is")
...
def main():
    ...
    mp.spawn(
        train_model,
        args=(world_size, args.batch_size, args.use_lora, args.epochs, args.lr, args.eval_steps, args.run_name, args.max_val_item_count),
        nprocs=world_size,
        join=True
    )
